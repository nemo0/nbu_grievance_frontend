...
const MY_DEPARTMENT_GRIEVANCES_URL = '/grievance/department/my';

const MyDepartmentGrievances = () => {
  ...
  useEffect(() => {
    ...
    const getGrievances = async () => {
      try {
        const response = await axiosPrivate.get(MY_DEPARTMENT_GRIEVANCES_URL, {
          signal: controller.signal,
        });
        console.log(response.data);
        const data = await response.data;
        isMounted && setGrievances(data);
      } catch (err) {
        console.error(err);
...
export default MyDepartmentGrievances;
